...
public final class GenListModuleReader extends AbstractXMLFilter {
  ...
  public void startElement(final String uri, final String localName, final String qName, final Attributes atts)
    ...
    if (!(cls != null && cls.isValid()) && !ELEMENT_NAME_DITA.equals(localName)) {
      if (nonDitaContext(classes)) {
        ...
        logger.info(
          MessageUtils
            .getMessage("DOTJ070I", atts.getValue(ATTRIBUTE_NAME_CLASS), localName)
            .setLocation(atts)
            .toString()
        );
      }
   ...
   */
  private void parseAttribute(final Attributes atts, final String attrName, final String scope) throws SAXException {
    ...
    if (ATTRIBUTE_NAME_HREF.equals(attrName)) {
      ...
      if (isFormatDita(attrFormat) && !isDitaMap() && !job.crawlTopics()) {
        ...
        (followLinks() && canFollow(attrValue)) ||
