  ...
  private void processFile(final Reference ref) throws DITAOTException {
    ...
    try {
      ...
      if (currentFile.equals(rootFile)) {
        throw new DITAOTException(MessageUtils.getMessage("DOTJ012F", params) + ": " + sax.getMessage(), sax);
      ...
      if (!exists(currentFile)) {
        ...
        throw new DITAOTException(
          MessageUtils.getMessage("DOTJ078F", params) + " Cannot load file: " + e.getMessage(),
          e
        );
      } else if (processingMode == Mode.STRICT) {
        throw new DITAOTException(
          MessageUtils.getMessage("DOTJ079E", params) + " Cannot load file: " + e.getMessage(),
          e
        );
      } else {
      ...
      if (currentFile.equals(rootFile)) {
        throw new DITAOTException(MessageUtils.getMessage("DOTJ012F", params) + ": " + e.getMessage(), e);
      } else if (processingMode == Mode.STRICT) {
